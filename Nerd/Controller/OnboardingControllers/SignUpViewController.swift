...
class SignUpViewController: UIViewController {
    ...
    @IBAction private func createAccountClick(_ sender: Any) {
        ...
        loginHandler.emailSignUp(email: emailText, password: passText, completion: { (errorMsg) in
            if let errorMsg = errorMsg {
                let popup = PopupView(frame: self.view.frame, text: errorMsg, image: UIImage(systemName: "exclamationmark.octagon.fill")!)
                self.view.addSubview(popup)
            }
        })
    }
    ...
    func performFBLogin() {
        ...
        login.logIn(permissions: ["public_profile", "email"], viewController: self, completion: {  (completion) in
    ...
    deinit {
        print("Deinit: Sign Up VC")
    }
}
// MARK: Extensions
extension SignUpViewController: LoginProtocol {
    func signedIn() {
        // once signed in present the main tab bar for the userw
        User.sharedInstance.userType = .registeredUser
        if User.sharedInstance.isNewUser {
            if let gradeVC = storyboard?.instantiateViewController(identifier: "gradeVC") as? GradeEntryViewController {
                gradeVC.modalPresentationStyle = .fullScreen
                self.navigationController?.setViewControllers([gradeVC], animated: true)
            }
        } else {
